...
def _build_pdf_bytes(data: dict[str, Any]) -> bytes:
    ...
    story.append(Paragraph(f"Compliance Scanner Platform • Scan #{data['scan_id']}", subtitle_style))
    ...
    story.append(
        Paragraph(
            ...
            "Generated automatically by Compliance Scanner AI.</i>",
            subtitle_style,
        )
    )
...
def _build_docx_bytes(data: dict[str, Any]) -> bytes:
    ...
    sub_run = sub_p.add_run(f"Compliance Scanner Platform • Scan #{data['scan_id']}")
    ...
    footer_run = footer_p.add_run(
        "Evaluated under Legal Metrology (Packaged Commodities) Rules 2011. Generated by Compliance Scanner AI."
    )
